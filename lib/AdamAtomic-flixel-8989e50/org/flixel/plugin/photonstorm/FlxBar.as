...
package org.flixel.plugin.photonstorm 
{
	...
	public class FlxBar extends FlxSprite
	{
		...
		private var canvasBarRect:Rectangle;
		...
		private var emptyColor:uint = 0x00005100;
		private var filledColor:uint = 0xff00F400;
		...
		private static const CANVAS_WIDTH:uint = 320;
		...
		public function FlxBar(x:int, y:int, direction:uint = FILL_LEFT_TO_RIGHT, width:int = 100, height:int = 10, parentRef:* = null, variable:String = "", min:Number = 0, max:Number = 100, border:Boolean = false):void
		{
			...
			canvas = new BitmapData(CANVAS_WIDTH, height, true, 0x0);
			...
			createFilledBar(emptyColor, filledColor, border);
		...
		public function setFillColor(color:uint) : void {
			filledColor = color;
			filledBar.fillRect(filledBar.rect, color);
		}
		...
		public function resize(width:int, height:int) : void
		{
			...
			canvas.fillRect(canvasBarRect, FlxG.TRANSPARENT);
			setRange(min, max);
			createFilledBar(emptyColor, filledColor, border);
			updateBar();
		}
...
//			}
		...
		public function createFilledBar(empty:uint, fill:uint, showBorder:Boolean = false, border:uint = 0xffffffff):void
		{
			...
			canvasBarRect = new Rectangle(0, 0, canvas.width, canvas.height);
		}
...
//			}
		...
		public function getFillDirection() : uint {
		...
		protected function didEmpty() : void
		{
			if (emptyCallback is Function)
			{
				emptyCallback.call();
			}
		}
		
		protected function didFill() : void {
			if (filledCallback is Function) 
			{
				filledCallback.call();
			}
		}
